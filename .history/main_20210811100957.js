...
function getDiff(d1, d2) {
  ...
  return `${yearDiff(d1, d2) ? yearDiff(d1, d2) + " " : ''}${monthDifference(d1, d2) ? monthDifference(d1, d2) + " " : ''}${totalDayDiff(d1, d2)}`;
}
...
function getNumberedMonthDifference(beforeDate, afterDate) {
  ...
  // get the literal day e.g is 18 < 24. If so, then -1 off return value. If not, do nothing
  const dayOffset = beforeDate.getDay() < afterDate.getDay() ? -1 : 0;
  console.log('beforeDate.getDay()', beforeDate.getDay());
  console.log('afterDate.getDay(', afterDate.getDay());
...
function yearDiff(d1, d2) {
  const years = Math.floor((d2.getTime() - d1.getTime()) / (1000*60*60*24*365));
