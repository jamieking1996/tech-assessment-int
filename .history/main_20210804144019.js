...
getDiff(d1, d2) {
  if (!this.isBeforeDateBeforeAfterDate(d1, d2)) {
    return new Error('First date is after second date.');
  }
  return `${this.yearDiff(d1, d2)} ${this.monthDifference(
    d1,
    d2
  )} ${this.totalDayDiff(d1, d2)}`;
}

monthDifference(beforeDate, afterDate) {
  const months = this.getNumberedMonthDifference(beforeDate, afterDate);
  return months <= 0 ? '' : `${months} ${this.pluralise(months, 'month')},`;
}

getNumberedMonthDifference(beforeDate, afterDate) {
  const months = beforeDate.getMonth() > afterDate.getMonth() ? 12 : 0;
  return months - beforeDate.getMonth() + afterDate.getMonth();
}
...
isBeforeDateBeforeAfterDate(beforeDate, afterDate) {
